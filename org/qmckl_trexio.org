...
#define MAX_STR_LEN 1024
  ...
  if (basis_type[0] == 'G' || basis_type[0] == 'N') {
...
*** AO Normalization
   #+begin_src c :tangle (eval c)
  {
   ...
   #+end_src
*** Number of numerical orbital spline data points

   #+begin_src c :tangle (eval c)
  int64_t numgrid_num = 0L;
  if (basis_type[0] == 'N') {

    rcio = trexio_read_basis_numgrid_num_64(file, &numgrid_num);
    if (rcio != TREXIO_SUCCESS) {
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_basis_numgrid_num",
                             trexio_string_of_error(rcio));
    }

    assert (numgrid_num > 0);
    rc = qmckl_set_ao_basis_numgrid_num(context, numgrid_num);
...
*** Number of numerical spline coefficient points

   #+begin_src c :tangle (eval c)
  int64_t interp_coeff_cnt = 0L;
  if (basis_type[0] == 'N') {

    rcio = trexio_read_basis_interp_coeff_cnt_64(file, &interp_coeff_cnt);
    if (rcio != TREXIO_SUCCESS) {
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
    ...
    assert (interp_coeff_cnt > 0);
    rc = qmckl_set_ao_basis_interp_coeff_cnt(context, interp_coeff_cnt);
...
*** Start indices of splines
  ...
  if (basis_type[0] == 'N'){
    ...
    int64_t* numgrid_start = (int64_t*) qmckl_malloc(context, mem_info);

    if (numgrid_start == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_numgrid_start_X",
                             NULL);
    }

    assert (numgrid_start != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_numgrid_start_64(file, numgrid_start, shell_num);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, numgrid_start);
      numgrid_start = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_basis_numgrid_start",
                             trexio_string_of_error(rcio));
    }

    /* Store data */
    rc = qmckl_set_ao_basis_numgrid_start(context, numgrid_start, shell_num);
...
*** Number of data points per spline
  ...
  if (basis_type[0] == 'N'){
    ...
    int64_t* numgrid_size = (int64_t*) qmckl_malloc(context, mem_info);

    if (numgrid_size == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_numgrid_size_X",
                             NULL);
    }

    assert (numgrid_size != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_numgrid_size_64(file, numgrid_size, shell_num);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, numgrid_size);
      numgrid_size = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_basis_numgrid_size",
                             trexio_string_of_error(rcio));
    }

    /* Store data */
    rc = qmckl_set_ao_basis_numgrid_size(context, numgrid_size, shell_num);
...
*** Radial values of spline data
  ...
  if (basis_type[0] == 'N'){
    ...
    double* numgrid_radius = (double*) qmckl_malloc(context, mem_info);

    if (numgrid_radius == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_numgrid_radius_X",
                             NULL);
    }

    assert (numgrid_radius != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_numgrid_radius_64(file, numgrid_radius, numgrid_num);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, numgrid_radius);
      numgrid_radius = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_basis_numgrid_radius",
                             trexio_string_of_error(rcio));
    }

    /* Store data */
    rc = qmckl_set_ao_basis_numgrid_radius(context, numgrid_radius, numgrid_num);
...
*** Kind of interpolator
   ...
   #+begin_src c :tangle (eval c)
  if (basis_type[0] == 'N'){
    char interpolator_kind[MAX_STR_LEN];

    rcio = trexio_read_basis_interpolator_kind(file, interpolator_kind, MAX_STR_LEN);
    if (rcio != TREXIO_SUCCESS) {
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_interpolator_kind",
                             trexio_string_of_error(rcio));
    }

    if (interpolator_kind[0] == 'P') {
      rc = qmckl_set_ao_interpolator_kind(context, interpolator_kind[0]); 
    } else {
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
                             "trexio_read_interpolator_kind",
...
*** Interpolator for wave function
  ...
  if (basis_type[0] == 'N'){
    ...
    /* Allocate array for data */
    int64_t data_size = numgrid_num * interp_coeff_cnt;
    mem_info.size = data_size * sizeof(double);

    double* interpolator_phi = (double*) qmckl_malloc(context, mem_info);

    if (interpolator_phi == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_interpolator_phi_X",
                             NULL);
    }

    assert (interpolator_phi != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_interpolator_phi_64(file, interpolator_phi, data_size);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, interpolator_phi);
      interpolator_phi = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
    ...
    /* Store data */
    rc = qmckl_set_ao_basis_interpolator_phi(context, interpolator_phi, data_size);
...
*** Interpolator for wave function gradient
  ...
  if (basis_type[0] == 'N'){
    ...
    double* interpolator_grad = (double*) qmckl_malloc(context, mem_info);

    if (interpolator_grad == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_interpolator_grad_X",
                             NULL);
    }

    assert (interpolator_grad != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_interpolator_grad_64(file, interpolator_grad, data_size);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, interpolator_grad);
      interpolator_grad = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
    ...
    /* Store data */
    rc = qmckl_set_ao_basis_interpolator_grad(context, interpolator_grad, data_size);
...
*** Interpolator for radial wave function laplacian
  ...
  if (basis_type[0] == 'N'){
    ...
    double* interpolator_lap = (double*) qmckl_malloc(context, mem_info);

    if (interpolator_lap == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_trexio_read_basis_interpolator_lap_X",
                             NULL);
    }

    assert (interpolator_lap != NULL);

    /* Read data */
    rcio = trexio_read_safe_basis_interpolator_lap_64(file, interpolator_lap, data_size);
    if (rcio != TREXIO_SUCCESS) {
      qmckl_free(context, interpolator_lap);
      interpolator_lap = NULL;
      return qmckl_failwith( context,
                             QMCKL_FAILURE,
    ...
    /* Store data */
    rc = qmckl_set_ao_basis_interpolator_lap(context, interpolator_lap, data_size);
